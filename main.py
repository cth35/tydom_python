...
def parse_response(bytes_str):
    response = response_from_bytes(bytes_str[1:])
...
def parse_put_response(bytes_str):
    ...
    response = put_response_from_bytes(bytes_str[1:])
    data = response.data.decode("utf-8")
    if (data != ''):
        parsed = json.loads(data)
        print(json.dumps(parsed, sort_keys=True, indent=4, separators=(',', ': ')))
...
async def get_devices_meta(websocket):
    str = "\x02GET /devices/meta HTTP/1.1\r\nContent-Length: 0\r\nContent-Type: application/json; charset=UTF-8\r\nTransac-Id: 0\r\n\r\n"
    a_bytes = bytes(str, "ascii")
    await websocket.send(a_bytes)
    name = await websocket.recv()
    parse_response(name)
...
async def get_devices_data(websocket):
    str = "\x02GET /devices/data HTTP/1.1\r\nContent-Length: 0\r\nContent-Type: application/json; charset=UTF-8\r\nTransac-Id: 0\r\n\r\n"
    a_bytes = bytes(str, "ascii")
    await websocket.send(a_bytes)
    name = await websocket.recv()
    parse_response(name)
...
async def get_confis_file(websocket):
    str = "\x02GET /configs/file HTTP/1.1\r\nContent-Length: 0\r\nContent-Type: application/json; charset=UTF-8\r\nTransac-Id: 0\r\n\r\n"
    a_bytes = bytes(str, "ascii")
    await websocket.send(a_bytes)
    name = await websocket.recv()
    parse_response(name)
...
async def put_devices_data(websocket):
    ...
    str_request = "\x02PUT /devices/10/endpoints/10/data HTTP/1.1\r\nContent-Length: "+str(len(body))+"\r\nContent-Type: application/json; charset=UTF-8\r\nTransac-Id: 0\r\n\r\n"+body+"\r\n\r\n"
...
async def hello():
    ...
    conn = http.client.HTTPSConnection("mediation.tydom.com", 443)
    # Get first handshake
    ...
    digestAuthHeader = digestAuth.build_digest_header('GET', "https://mediation.tydom.com:443/mediation/client?mac={}&appli=1".format(mac))
    ...
    websocketHeaders = {'Authorization': digestAuthHeader, 'Sec-WebSocket-Key': str(encoded),  'Accept': '*/*', 'Sec-WebSocket-Version': '13'}
